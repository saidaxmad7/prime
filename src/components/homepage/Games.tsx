                        ...
                        {data?.map((game) => (
                            <SwiperSlide key={game.id}>
                                <div
                                    ...
                                    className="cursor-pointer relative rounded-2xl overflow-hidden group bg-black"
                                >
                                    <img
                                        src={game.img}
                                        alt={game.name}
                                        className="w-full h-60 object-contain transition-transform duration-300 group-hover:scale-105 bg-black"
                                    />
                    ...
                    <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {data?.map((game) => (
                            <div
                                key={game.id}
                                onClick={() => router.push(`/games/${game.id}`)}
                                className="bg-[#1c1c28] rounded-xl overflow-hidden shadow-lg hover:scale-105 transition-transform cursor-pointer"
                            >
                                <img
                                    src={game.img}
                                    alt={game.name}
